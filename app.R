...
ui <- fluidPage(
  useShinyjs(),
  titlePanel("Grabador y Conversor de Audio a Texto"),
  
  # Sección de Grabación de Audio
  tags$h2("Grabación de Audio"),
  tags$button(id = "startBtn", "Iniciar Grabación", onclick = "startRecording();"),
  tags$button(id = "stopBtn", "Detener Grabación", onclick = "stopRecording();"),
  tags$br(), tags$br(),
  tags$audio(id = "audioPlayback", controls = TRUE),
  tags$br(), tags$br(),
  tags$a("Descargar Audio Grabado", id = "downloadLink", href = "#", download = "grabacion_audio.webm", target = "_blank"),
  tags$hr(),
  
  # Sección de Conversión de Audio a Texto
  tags$h2("Conversión de Audio a Texto"),
  textInput("apiKey", "Ingresa tu API Key de Google sCloud:", placeholder = "TU_API_KEY"),
  fileInput("audioFile", "Selecciona un archivo de audio (.wav, .mp3, .flac, .webm)"),
  actionButton("convertFileButton", "Convertir Audio de Archivo a Texto"),
  actionButton("convertRecordedButton", "Convertir Audio Grabado a Texto"),
  tags$br(),
  textOutput("transcription"),
  verbatimTextOutput("error"),
  verbatimTextOutput("request_json"),
  
  # Código JavaScript para la Grabación de Audio (MODIFICADO)
  tags$script(HTML("
    var mediaRecorder;
    var recordedChunks = [];
      ...
      navigator.mediaDevices.getUserMedia({ audio: true })
        .then(function(stream) {
          ...
          mediaRecorder.onstop = function() {
            ...
            var audioURL = window.URL.createObjectURL(recordedAudioBlob);
            document.getElementById('audioPlayback').src = audioURL;
            var downloadLink = document.getElementById('downloadLink');
            downloadLink.href = audioURL;
            downloadLink.download = 'grabacion_audio.webm';
            console.log('Grabación detenida');
            recordedChunks = [];

            // **NUEVO: Convertir a Base64 y enviar a Shiny INMEDIATAMENTE al detener la grabación**
            var reader = new FileReader();
            reader.onloadend = function() {
              var base64String = reader.result.split(',')[1];
              Shiny.setInputValue('recordedAudioBase64', base64String); // Enviar a Shiny
            }
            reader.readAsDataURL(recordedAudioBlob); // Leer como Data URL
          }
        })
        .catch(function(err) {
    ...
    // **ELIMINADA Función convertRecordedAudio() y el listener asociado en jQuery**
    // La conversión ahora se inicia directamente en onstop de mediaRecorder
  "))
)
