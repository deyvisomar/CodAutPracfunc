...
###
py_safe_run <- function(python_code_lines) {
  if (is.character(python_code_lines)) {
    code <- paste(python_code_lines, collapse = "\n")
    py_run_string(code)
  } else {
    stop("El argumento debe ser un vector de strings.")
  }
}
py_safe_run(codigo_cs_code)
codigo_cs_code<- '
import os
import win32com.client
...
# Limpia el código pegado (copiado desde VS Code o similar)
py_clean_clipboard <- function() {
  raw_code <- readLines("clipboard")
  cleaned_code <- gsub("\r", "", raw_code)  # limpia retorno de carro (Windows)
  return(cleaned_code)
}
codigo<-py_clean_clipboard()
py_safe_run(codigo_cs_code)
## correo ##
...
py_clipboard_to_paste <- function() {
  lines <- readLines("clipboard")
  escaped <- gsub('"', '\\\\"', lines)
  quoted <- sprintf('  "%s"', escaped)
  result <- c("codigo <- paste(", paste(quoted, collapse = ",\n"), '  , sep = "\\n")')
  cat(result, sep = "\n")
}
codigo <-py_clipboard_to_paste()
codigo <- paste(
  "import os",
  "import win32com.client",
  "",
...
convertir_python_para_r <- function() {
  ...
  # Unir en un paste() de R
  salida <- c("codigo_python <- paste(", paste0("  ", lineas_citadas, collapse = ",\n"), '  , sep = "\\n")')
